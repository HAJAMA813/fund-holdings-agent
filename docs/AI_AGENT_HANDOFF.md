...
当前项目的核心是一个“确定性工作流 Agent”：任务拆解与编排、工具调用、状态记录、披露等待、断点续跑、有限重试、异常分流、人工确认和多格式交付。

在此之上已增加一个**可选的大模型层**（2026-08-17 接入 DeepSeek）：单轮问答与摘要（`fund-agent-ask`）、多步工具编排 Agent（`fund-agent-agent`，ReAct 循环、4 个只读工具）、以及 Mac 菜单中的自动解读。该层**只读**已验证 JSON，只做解释与编排，不参与任何事实与数值计算；未配置 API Key 时自动降级为确定性摘要，确定性管道不 import 该层（有测试保证解耦）。

不要把规则计算描述成模型推理；同样也不要把大模型层描述成决策主体——**规则决定“是什么”，模型只决定“怎么呈现和怎么编排查询”**。基金名单、持仓事实、行业分类、比例计算和异常判定必须继续由确定性代码完成。
...
- 大模型层（可选）：单轮问答与摘要、多步工具编排 Agent（ReAct，4 个只读工具）、菜单内自动解读；无 Key 时自动降级。
- 公开版脱敏：内部机构与人员数据由 `.gitignore` 排除，仓库内无敏感信息。
- 已使用徐小勇2025Q4至2026Q2真实数据生成并检查Excel和8页PDF。
- 自动测试基线：转交时101项通过；加入大模型问答与多步 Agent 层后，当前为121项。
...
6. 大模型层已接入（只读、可选），但仍属**有界 Agent**：无显式规划、无反思循环、无跨会话记忆，工具全部只读。
